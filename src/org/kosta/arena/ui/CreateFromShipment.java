...
public abstract class CreateFromShipment extends CreateFrom
{
	...
	protected ArrayList<KeyNamePair> loadInvoiceData (int C_BPartner_ID)
	{
		...
		//
		StringBuffer sql = new StringBuffer("SELECT i.C_Invoice_ID,").append(" i.DocumentNo ") // add by aries
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		sql.append(" WHERE l.C_Order_ID=? " +
				" and coalesce(l.qtyordered,0 ) > coalesce(l.qtyDelivered, 0 ) "			//	#1
		...
		try
		{
			...
			while (rs.next())
			{
				...
				// Add locator
				line.add(getLocatorKeyNamePair(rs.getInt(5)));// 3-Locator edit by AH
				data.add(line);
			}
		}
		catch (SQLException e)
	...
	protected void configureMiniTable (IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(6, String.class, true);     //  Ship
		miniTable.setColumnClass(7, String.class, true);   //  Invoice
		miniTable.setColumnClass(8, String.class, false);  //  Locator
		//  Table UI
		miniTable.autoSize();
		...
		*/
		...
		for (int i = 0; i < miniTable.getRowCount(); i++)
		{
			if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue()) {
				...
				pp = (KeyNamePair) miniTable.getValueAt(i, 8); // Locator
				...
				int M_RMALine_ID = 0;
				pp = (KeyNamePair) miniTable.getValueAt(i, 6); // RMA
				// If we have RMA
				if (pp != null)
					M_RMALine_ID = pp.getKey();
				int C_InvoiceLine_ID = 0;
				MInvoiceLine il = null;
				pp = (KeyNamePair) miniTable.getValueAt(i, 7); // InvoiceLine
				if (pp != null)
