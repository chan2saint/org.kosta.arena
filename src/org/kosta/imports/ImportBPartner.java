...
implements ImportProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		//	Set Payment Term
		sql = new StringBuilder ("UPDATE I_BPartner i ")
				.append("SET c_paymentterm_id =(SELECT c_paymentterm_id FROM c_paymentterm g")
				.append(" WHERE i.paymentterm like g.name AND g.AD_Client_ID IN ( i.AD_Client_ID)) ")
				.append("WHERE c_paymentterm_id IS NULL")
				.append(" AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.FINE)) log.fine("Set Payment Term=" + no);
			...
			{	
				...
				if ( ! New_BPValue.equals(Old_BPValue)) {
					...
					if (impBP.getC_BPartner_ID() == 0)	//	Insert new BPartner
					{
						...
						// add by aries
						Integer C_PaymentTerm_ID = impBP.get_ValueAsInt("C_PaymentTerm_ID");
						if ( C_PaymentTerm_ID > 0){
							bp.setC_PaymentTerm_ID(C_PaymentTerm_ID);
						}
						// end 
