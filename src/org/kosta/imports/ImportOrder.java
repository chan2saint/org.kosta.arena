...
public class ImportOrder extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		sql = new StringBuilder ("UPDATE I_Order ")
			  ...
			  .append(" AD_Org_ID = COALESCE (").append(m_AD_Org_ID).append(", 0").append ("),")
		...
		*/
		...
		sql = new StringBuilder ("UPDATE I_Order o ")
			  .append("SET M_Product_ID=(SELECT MAX(M_Product_ID) FROM M_Product p")
			  .append(" WHERE p.value = (productcategory_value || '-' || productvalue) AND o.AD_Client_ID=p.AD_Client_ID) ")
			  .append(" WHERE M_Product_ID IS NULL AND ProductValue IS NOT NULL and productcategory_value is not null ")
			  .append(" AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		// add modifier 
		sql = new StringBuilder ("UPDATE I_Order o ")
			  .append("SET UM_Modifier_ID =(SELECT UM_Modifier_ID FROM UM_Modifier c")
			  .append(" WHERE o.UM_modifier_value =c.value AND o.AD_Client_ID=c.AD_Client_ID) ")
			  .append("WHERE o.UM_modifier_value is not null AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		// end 
				
		// bank account
		sql = new StringBuilder ("UPDATE I_Order o ")
			  .append("SET C_BankAccount_ID =(SELECT C_BankAccount_ID FROM C_BankAccount c")
			  .append(" WHERE o.C_BankAccount_Value = c.value AND o.AD_Client_ID=c.AD_Client_ID) ")
			  .append("WHERE o.C_BankAccount_Value is not null AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		try
		{
			...
			while (rs.next ())
			{
					...
					|| !oldDocumentNo.equals(cmpDocumentNo))
				{
					...
					//if ( imp.get_ValueAsInt("C_BankAccount_ID") != 0)
						//order.set_ValueNoCheck("C_BankAccount_ID", imp.get_ValueAsInt("C_BankAccount_ID"));
					// end
				...
				// add by ah
				// set service charge
				line.set_ValueNoCheck("UM_Modifier_ID", imp.get_ValueAsInt("UM_Modifier_ID"));
				if (imp.getC_Charge_ID() != 0){
					line.setPriceEntered(imp.getPriceActual());
					line.setPriceList(imp.getPriceActual());
					line.setPriceLimit(imp.getPriceActual());
					line.setLineNetAmt(imp.getPriceActual());
				}
				// end
				line.saveEx();
