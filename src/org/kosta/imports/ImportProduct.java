...
public class ImportProduct extends SvrProcess implements ImportProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		// add by aries
		sql = new StringBuilder ("UPDATE I_Product i ")
			.append("SET M_Product_Category_ID = (SELECT M_Product_Category_ID FROM M_Product_Category u WHERE u.value =i.productcategory_value AND u.AD_Client_ID IN (i.AD_Client_ID)) ")
			.append(" where I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (log.isLoggable(Level.INFO)) log.info("Set M_Product_Category_ID=" + no);
		...
		sql = new StringBuilder ("UPDATE I_Product i ")
			.append("SET X12DE355 = ")
			.append("(SELECT MAX(X12DE355) FROM C_UOM u WHERE u.IsDefault='Y' AND u.AD_Client_ID IN (i.AD_Client_ID)) ")
		...
		//
		sql = new StringBuilder ("UPDATE I_Product i ")
			.append("SET C_UOM_ID = (SELECT C_UOM_ID FROM C_UOM u WHERE u.X12DE355=i.X12DE355 AND u.AD_Client_ID IN (i.AD_Client_ID)) ")
