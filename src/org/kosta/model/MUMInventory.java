 ...
 */
public class MUMInventory extends X_M_Inventory implements DocAction
{
	/**
	 * 
	 */
	 ...
	 *	@return MInventory
	 */
	public static MUMInventory get (Properties ctx, int M_Inventory_ID)
	{
		Integer key = new Integer (M_Inventory_ID);
		MUMInventory retValue = (MUMInventory) s_cache.get (key);
		if (retValue != null)
			return retValue;
		retValue = new MUMInventory (ctx, M_Inventory_ID, null);
	...
	/**	Cache						*/
	private static CCache<Integer,MUMInventory> s_cache = new CCache<Integer,MUMInventory>(Table_Name, 5, 5);
	 ...
	 */
	public MUMInventory (Properties ctx, int M_Inventory_ID, String trxName)
	{
	 ...
	 *	@param trxName transaction
	 */
	public MUMInventory (Properties ctx, ResultSet rs, String trxName)
	{
	 ...
	 */
	public MUMInventory (MWarehouse wh)
	{
	 ...
	 * @param trxName
	 */
	public MUMInventory (MWarehouse wh, String trxName)
	{
	...
	public String completeIt()
	{
		...
		if (DOCSUBTYPEINV_MOveIn.equals(docSubTypeInv) ){
			MUMInventory inv = new MUMInventory(getCtx(), get_ValueAsInt("M_Inventory_Ref"), get_TrxName());
			
			inv.set_ValueNoCheck("M_Inventory_Ref", inv.get_ID());
			inv.saveEx();
		}		
		//
		setProcessed(true);
	...
	public boolean reverseCorrectIt()
	{
		...
		MUMInventory reversal = reverse(false);
		if (reversal == null)
			return false;
	...
	private MUMInventory reverse(boolean accrual) {
		...
		//	Deep Copy
		MUMInventory reversal = new MUMInventory(getCtx(), 0, get_TrxName());
		...
		for (int i = 0; i < oLines.length; i++)
		{
			...
			MUMInventoryLine rLine = new MUMInventoryLine(getCtx(), 0, get_TrxName());
	...
	public boolean reverseAccrualIt()
	{
		...
		MUMInventory reversal = reverse(true);
		if (reversal == null)
			return false;
