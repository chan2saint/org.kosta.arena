...
public class BankRegister extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		m_parameterWhere.append(" And ba.C_BankAccount_ID = " + p_C_BankAccount_ID); // note
	...
	private void createBalanceLine()
	{
		StringBuffer sb = new StringBuffer ("INSERT INTO T_BankRegister "
			...
			+ "DateAcct, C_BankAccount_ID, BankName, C_BPartner_ID, BPartner, DocumentNo," // Account, 
		...
		sb.append("SELECT ").append(getAD_PInstance_ID()).append(", ").append(Env.getAD_Client_ID(getCtx())).append(", ").append(Env.getAD_Org_ID(getCtx())).append(", ")
			.append(DB.TO_DATE(p_DateAcct_From, true)).append(", ").append(p_C_BankAccount_ID).append(", ")
	...
	private void createDetailLines()
	{
		StringBuffer sb = new StringBuffer ("INSERT INTO T_BankRegister "
				+ "(AD_PInstance_ID, AD_Client_ID, AD_Org_ID, "
				+ "DateAcct, C_BankAccount_ID, BankName, C_BPartner_ID, BPartner, DocumentNo, " //Account, 
				+ "AmtAcctDr, AmtAcctCr, Balance, Account) ");

		sb.append("SELECT DISTINCT ").append(getAD_PInstance_ID()).append(", fa.AD_Client_ID, fa.AD_Org_ID, ")
			.append("TRUNC(fa.DateAcct), ba.C_BankAccount_ID, b.name, bp.C_BPartner_ID, bp.name, p.documentNo, "	//ev.name, 
			+ "fa.AmtAcctDr, fa.AmtAcctCr, arena_saldo_account( ba.C_BankAccount_ID, fa.DateAcct, fa.Fact_Acct_ID)+(fa.AmtAcctDr-fa.AmtAcctCr), fa.Fact_Acct_ID "
			+ "From Fact_Acct fa "
			+ "Inner Join C_Payment p On p.C_Payment_ID = fa.Record_ID AND p.docstatus IN ('CO', 'CL') "
			+ "Inner Join C_BankAccount ba On ba.C_BankAccount_ID = p.C_BankAccount_ID "
			+ "Inner Join C_Bank b On ba.C_Bank_ID = b.C_Bank_ID "
		...
		//Yvonne 8/7/2009: include all transactions where Bank Asset account and Bank In-Transit account
		sb.append(" order by fa.AD_Client_ID, fa.AD_Org_ID, b.name, TRUNC(fa.DateAcct), fa.Fact_Acct_ID"); // remark by ah payment Document No, remark by Surya bp.name
